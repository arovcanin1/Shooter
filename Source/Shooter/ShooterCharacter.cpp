	...
	StunChance(.25f)
{
	...
	CameraBoom->TargetArmLength = 180.f; // The camer follows at this distance behind the character
	...
	// Do not rotate when the controller rotate, let the controller only affect the camera 
...
bool AShooterCharacter::GetBeamEndLocation(const FVector& MuzzleSocketLocation, FHitResult& OutHitResult)
{
	...
	// Perform a secont trace, this time from the gun barrel 
...
void AShooterCharacter::EquipWeapon(AWeapon* WeaponToEquip, bool bSwapping)
{
	if (WeaponToEquip)
	{
		...
		if (HandSocket)
		{
			// Attacht the Weapon to the hand socket RightHandSocket
...
void AShooterCharacter::FinishReloading()
{
	...
	{
		// Amount of ammo the Character is carrying of the EquippedWeaponType
...
void AShooterCharacter::GetPickupItem(AItem* Item)
{
	...
	if (Weapon)
	{
		...
		{
			// Inventory full. Swap with Equipped Weapon
			SwapWeapon(Weapon);
		}
